...
def check_data_in_table_1(**kwargs):
    ...
    for row in results:
        if row[0] > 0:
            return 'check_data_in_table_2'
    return 'retry_check_table_1'

def check_data_in_table_2(**kwargs):
    client = bigquery.Client()
    query = "SELECT COUNT(1) FROM `your_project.your_dataset.table_2` WHERE <your_condition>"
    query_job = client.query(query)
    results = query_job.result()
    for row in results:
        if row[0] > 0:
            return 'send_email'
    return 'check_data_in_table_2'  # Keeps retrying until data is found
...
with DAG(
    ...
    check_data_in_table_2 = BranchPythonOperator(
        task_id='check_data_in_table_2',
        python_callable=check_data_in_table_2,
        provide_context=True,
    )
    ...
    check_data_in_table_1 >> check_data_in_table_2
    check_data_in_table_1 >> retry_check_table_1 >> check_data_in_table_1
    check_data_in_table_2 >> send_email_task >> end
    check_data_in_table_2 >> check_data_in_table_2  # Retries check for table 2 until successful
